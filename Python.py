...
#HTTPS - HYPER TEXT TRANSFER PROTOCOL SECURITY,SSL- ENCRYPTED[Means Secure Website]
from flask import Flask,request,Response #Importing Flask

app=Flask(__name__)#This is an object that will represent our website
#The (__name__) tell python that the file we are working on is our main file.

@app.route("/") #This is a root decorator saying that when someone try to visit home page run the code below it.
def home():
...
def submit():
    if request.method  == "POST":  #Sensitive data should besent using "POST" and it keep it hidden but get keep the data in the URL.
        #request.method -used for getting the method
        return "You have submitted the data."
    else:
        return "There was an error submitting the request."
    
if __name__ =="__main__":  #Important line to run the code
    app.run(debug =True)
